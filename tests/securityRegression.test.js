...
test('checkout simulado é explicitamente bloqueável e sincronização valida proprietário', () => {
  assert.match(server, /if \(!isSimulatedBillingAllowed\(\)\)/);
  assert.match(server, /expectedUserId: req\.user\.sub/);
...
test('auditoria de site usa status de validação e limite de entrada', () => {
  assert.match(server, /String\(req\.body\.site \|\| ''\)\.trim\(\)\.slice\(0, 2048\)/);
  assert.match(server, /res\.status\(error\.statusCode \|\| 500\)/);
});
